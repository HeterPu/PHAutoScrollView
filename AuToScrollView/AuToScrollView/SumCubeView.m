...
-(NSArray *)titleImageArray {
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"HuiJinMenu" ofType:@"plist"];
...
-(void)layoutSubviews {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            cube.delegate1 = self;
            cube.isRDragble = YES;
            cube.backgroundColor = [UIColor blackColor];
...
-(void)saveChangeToPlist {
    ...
    NSString *filepath = [[NSBundle mainBundle] pathForResource:@"HuiJinMenu" ofType:@"plist"];
